...
var (
	...
	ct = "32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd825" +
		"7bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f" +
		"52f904"
)

func main() {
	bcs := make([][]byte, len(cs))
	for i, c := range cs {
		bc, err := hex.DecodeString(c)
		if err != nil {
			panic(err)
		}
		bcs[i] = bc
	...
	for i := range bcs {
		for j := range bcs[i+1:] {
			...
			for k := 0; k < minLen; k++ {
				...
				potKey[k] = append(potKey[k], bc1[k]^' ', bc2[k]^' ')
			}
		}
	}

	key := make(map[int]byte)
	for k, v := range potKey {
		key[k] = mode(v)
	}

	bct, err := hex.DecodeString(ct)
	if err != nil {
		panic(err)
	}
...
func mode(s []byte) byte {
	var (
		max byte
		ctr = make(map[byte]int)
	)
